...
static const unsigned int ROOM_MAX_SIZE = 10;
static const unsigned int ROOM_MIN_SIZE = 4;
...
void Map::makeRoom(bool first, int x1, int y1, int x2, int y2) {
    dig(x1, y1, x2, y2);
    if (first) {
        ...
        if (rng->getInt(0, 3) == 0) {
            engine.actors.push(new Actor((x1 + x2) / 2, (y1 + y2) / 2, '@', TCODColor::amber));
        }
    }
}
